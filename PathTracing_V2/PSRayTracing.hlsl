...
float getRand(int seed, int iter)
{
...
void IntersectPrisma
	...
	Hit bestHit, float3 pos, float3 size, float3 albedo, float3 emission, float3 specular = 0, float smooth = 0, float4 refractEta = 0)
{
	IntersectQaud(ray, bestHit, float3(-size.x, 0, -size.z) + pos, float3(size.x, 0, -size.z) + pos, float3(size.x, 0, size.z) + pos, float3(-size.x, 0, size.z) + pos, 0.8f, 0, 0, 0, refractEta); // Пол
	IntersectQaud(ray, bestHit, float3(size.x, 0, size.z) + pos, float3(size.x, 0, -size.z) + pos, float3(size.x, size.y * 2, -size.z) + pos, float3(size.x, 0, size.z) + pos, Colorizate(109, 102, 172), 0, 0, 0, refractEta); // Право
	IntersectQaud(ray, bestHit, float3(-size.x, 0, -size.z) + pos, float3(-size.x, 0, size.z) + pos, float3(-size.x, 0, size.z) + pos, float3(-size.x, size.y * 2, -size.z) + pos, Colorizate(175, 101, 111), 0, 0, 0, refractEta); // Лево
	IntersectQaud(ray, bestHit, float3(-size.x, 0, size.z) + pos, float3(size.x, 0, size.z) + pos, float3(size.x, size.y * 2, -size.z) + pos, float3(-size.x, size.y * 2, -size.z) + pos, 0.8f, 0, 0, 0, refractEta); // Верх
	IntersectQaud(ray, bestHit, float3(size.x, 0, -size.z) + pos, float3(-size.x, 0, -size.z) + pos, float3(-size.x, size.y * 2, -size.z) + pos, float3(size.x, size.y * 2, -size.z) + pos, 0.05f, 0, 0, 0, refractEta); // Зад
}

Hit Trace(Ray ray, inout float seed, float iter)
{
	Hit hit = InitHit();
	...
	//IntersectCube(ray, hit, pos, cubeSize, 0.8f, 0, 0, 0, 0);
	float size = 0.5f;
	//IntersectQaud(ray, hit, float3(-size, cubeSize.y * 2 - 0.001f, size) + pos, float3(size, cubeSize.y * 2 - 0.001f, size) + pos, float3(size, cubeSize.y * 2 - 0.001f, -size) + pos, float3(-size, cubeSize.y * 2 - 0.001f, -size) + pos, 0.8f, 15, 0, 0);
	float sizek = 4;
	float3 posk = float3(0, 0, 0);

	IntersectQaud(ray, hit, float3(-sizek, 0, -sizek) + posk, float3(sizek, 0, -sizek) + posk, float3(sizek, 0, sizek) + posk, float3(-sizek, 0, sizek) + posk, 0.8f, 0, 0, 0); // Пол
	...
	hit)
{
	...
	else
	{
		...
		return HDRtexture.SampleLevel(objSamplerState, float2(phi / (2 * PI), theta / PI), 0) * 0.2f;
	}
}
float3 initPixelPos
...
float4 main(Input input) : SV_TARGET
{
	...
	viewDiraction = initPixelPos(_PixelPos + float3((rand() * 2) - 1, (rand() * 2) - 1, (rand() * 2) - 1) * 0.0012f, float2(cameraAngle.x + 0.5f , cameraAngle.y - 0.5f), 1, screenResolution.w);
	Ray ray = InitRay(normalize(viewDiraction), cameraPos + float3(-1, 3, -3));
	float seed = 256;
	for (int i = 0; i < iterCount; i++)
	{
		Hit hit = Trace(ray, seed, i);
